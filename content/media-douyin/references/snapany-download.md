...
3. Click that arrow and choose the visible menu item `备用下载地址`. Do not open the normal signed media URL directly: it may open a media tab that the browser blocks or may expire.
4. If the backup action opens another result page, wait for it to load, locate the same video card, open its arrow again, and wait for a browser download event while activating the visible `备用下载地址` menu item. A new tab or a media preview is not proof that a local file was downloaded.
