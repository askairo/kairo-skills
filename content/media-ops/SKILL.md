...
- X 的 `scheduled_run` 必须调用 `media-x-api`，只使用官方 API；人工触发的 `interactive_run` 才能调用 `media-x`，并停留在用户确认后的受控会话。不得因 API 权限、套餐、限流或安全验证失败而回退到无人值守浏览器脚本。
...
- 外部下载必须通过页面可见的下载/备用地址控件完成；不能把页面上的签名直链、媒体预览页或旧下载目录文件当作已下载素材。平台子技能必须完成音视频轨和可播放性验收后才能上传。
