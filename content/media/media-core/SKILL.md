...
每个 pipeline 的来源生产调度器必须是唯一来源事实写入者；平台专属发布定时器不得直接创建候选或推进内容游标。统一分发扫描器是唯一消费 `ready` 目标的调度入口。发现多个活动调度器、旧平台定时器仍在运行或本地记录与内容层游标冲突时，先写入 `scheduler_authority_mismatch`，停止写入，不用旧平台队列猜测下一项。
