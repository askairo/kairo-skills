...
涉及浏览器的诊断、只读核验或恢复编排时，只向 `media-ops` 和平台技能传递 `playwright-mcp`；通过 Playwright MCP Bridge 接管已授权 Chrome Tab 是整个媒体运营体系唯一允许的浏览器执行通道。`media-loop` 不自行接管页面，也不得建议或批准 Computer Use、controlled-browser-session、CDP 或其他浏览器接口作为回退。
