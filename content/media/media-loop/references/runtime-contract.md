...
如果只读核验发现本地 `pending` 目标其实已在更早的运行中发布，先以 `historical_result_reconciled` 修复该目标、台账和游标。该检查点必须保留平台实际发布时间与 `newPublishSideEffectsThisRun: 0`，只表示历史阶段补记完成；它不把当前运行推进到 `published`，也不消耗当前运行的发布批量保护。当前触发仍要求推进一次发布时，回到更新后的 ready 扫描；库存为空则进入有界供给恢复。用户要求“今天发布”时，用账号配置时区判断自然日，前一日作品不能满足本日请求。
