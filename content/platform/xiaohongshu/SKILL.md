...
- 固定宽度为 `1254 px`，高度由封面实际内容计算；四边必须保留 `26 px` 米白外边，内容不得越过外边或圆角卡片。
- 固定几何：主体卡片 `x=26, y=26, w=1202, h=H-52, rx=24`；正文安全区 `x=110` 至 `x=1144`；右上双圆固定在卡片右上并允许被卡片边界裁切；分隔线固定从 `x=110` 到 `x=984`。
...
- 内容与背景保持安全距离：主题、分隔线、辅助句均不得贴近卡片边缘；辅助句最后一行之后至少保留 `188 px` 卡片内留白，再加底部 `26 px` 外边。高度公式为 `H = 内容最后一行基线 + 188 + 26`，并向上取整到 2 px。
- 封面只承诺正文解释的一个问题；发布前必须校验宽度、动态高度、四边白边、圆角、六个固定色值、字号层级、两行上限和底部安全距离。
