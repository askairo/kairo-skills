...
- 交付 3 个标题候选，且分别来自不同的吸引力路径：具体冲突/反差、人物动作或场景、读者可获得的判断；不得只替换同一模板中的一两个词。人物名不是固定前缀，只有确实增强识别和搜索时才放入标题。
- 标题必须在发布前校验为不超过 20 个字符（包括标点和 emoji）；编辑器出现 `超过 20`、`X / 20` 或类似校验提示时，先压缩标题再进入发布动作。
- 生成前读取目标账号最近至少 5 条标题和正文摘要；连续两条不得复用同一标题句式、开头动作、正文骨架或结尾方式。若本机配置提供 `copyRotation`，同时执行其中的窗口和轮换规则。
- 开头第一句就给出具体场景、矛盾、意外判断或实际后果，2—3 句内再落到人物事实和读者收益；禁止先写平台背景、账号背景、内容提要或空泛赞美。
- 正文只围绕一个核心问题，但不固定套用“三步判断”。根据素材在“对话切入、反常识转折、选择冲突、案例拆解、纠正常见误读”等结构中择一；事实/原意、编辑判断和读者启发仍须清楚分开。
- 保留编辑者的存在感：可以使用“我更在意的是”“换个角度看”等有限判断或轻微幽默，但不得虚构亲历、情绪或替人物下结论。用自然口语和具体动词替代报告式标签。
- 优先提供可收藏的判断清单、思考步骤、对比或避坑建议，但允许把它自然写进叙事，不强制每篇都编号。每张卡片只承载一个信息单元，卡片文字与正文结论一一对应。
- 人物观点系列可采用“上下文一句话 → 人物原话/原意 → 理念解释 → 一句引导”的短结构；人物、来源披露与领域限制均以内容资产和账号外部策略为准。
- 封面承诺必须由正文兑现；来源、日期、事实与个人判断清晰分开。
- 结尾根据内容在“明确带走一句、回扣开头、一个具体问题”中轮换；互动问题不是必填项，不为互动强行把陈述句改成问句。
